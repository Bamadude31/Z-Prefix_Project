...
export const EditItem = () => {
  ...
  const fetchTreeData = (id) => {
    console.log("test");
    fetch(
      `http://localhost:3001/preview?username=${encodeURIComponent(
        userData.username
  ...
  const postNewLink = (title, url) => {
    const queryParams = `?title=${encodeURIComponent(
      title
    )}&url=${encodeURIComponent(url)}&treeId=${encodeURIComponent(
      treeToEdit.id
    )}`;
    fetch(`http://localhost:3001/edit${queryParams}`, {
      ...
      if (res.ok) fetchTreeData(treeToEdit.id);
    });
  };

  const postNewTree = () => {
    const queryParams = `?header=${encodeURIComponent(
      header
    )}&subheader=${encodeURIComponent(
      subheader
    )}&treeEndpoint=${encodeURIComponent(
      treeEndpoint
    )}&bgimage=${encodeURIComponent(backgroundImage)}&userId=${
      userData.id
